...
@ContextConfiguration(classes = {BookingController.class, BookingService.class})
public class BookingControllerBase {

    private static final UUID USER_ID_OK = UUID.fromString("f14e9d07-a7f8-42bc-87e6-be8d1ffde7d1");
    private static final ZonedDateTime NOW = ZonedDateTime.from(ISO_OFFSET_DATE_TIME.parse("2020-06-14T10:54:27.6374357+02:00"));
    @Autowired
    ...
    public void setUp() {
        ...
        when(bookingService.getBookingsForClient(eq(USER_ID_OK)))
    ...
    private static Booking booking(final String description) {
        return Booking.builder().clientId(USER_ID_OK).creationTime(NOW).description(description).amount(amount()).build();
    }
